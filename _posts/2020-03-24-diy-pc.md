...
I've been a 8-year macOS user and often want to bring something new into my daily workflow - and now it's time. Here are the main ideas I followed and the hardware components I decided to go with after a quick 3-day research. I will use a bullet-point style to explain what I learned and some caveats to pay attention to; it may be good to keep it concise because most information can be easily searched on the Interent - as long as you know what to search for.

## Main Ideas

- Use  the latest generation, #1/#2-ranked components to build a high-end, *consumer-grade* desktop.

- Use components from the same, high-end brand as much as possible for the best compatibility and to save yourself a headache in the future. Here we use components from Asus and Corsair. 
...
- Choose between Intel and AMD.
  ...
  - Official guide on [understanding Intel CPU's model numbers](). Some Interesting high-end models that worth a look:
...
- Do you prioritize on single-thread performance or the number of cores? (Hint: it's a tradeoff - processors that are better in one will be not as good in the other)
  ...
  - It's very hard for a program to effectively use multiple cores. 
    ...
    ![diy-pc-paint-fence](/Users/max/Documents/CS-Projects/maxwyb.github.io/assets/diy-pc-paint-fence.png)
    ...
    - Even for programs that can effectively use multiple cores, adding more cores does not necessarily improve the experience in all parts of this app. Take Adobe Lightroom Classic for example: export and apply presets in batch use many cores but most other operations such as scrolling through the library/pictures and real-time development on a single picture are still single-threaded. If you prioritize on experiences of interacting with the software and don't mind spending an extra 30 seconds on exporting 1000 pictures, it's still a better idea to focus on single-thread performance.
    ...
    ![diy-pc-adobe-premiere-multicore](/Users/max/Documents/CS-Projects/maxwyb.github.io/assets/diy-pc-adobe-premiere-multicore.jpg)
...
- How much expandability do you need on the motherboard? 
- Then, go to a benchmark site to find performance ranking of all CPUs on the market or performance data of a CPU model. 
  ...
  - [Articles](https://www.pugetsystems.com/all_articles.php) on Puget Systems: great benchmarks of hardware on photography & video editing applications. Make sure to read their latest articles as applications are better optimized for moderm CPUs over the years.
...
- Filter the models with the CPU's *Socket* and the corresponding *Chipset*. Only those motherboards work with this CPU - in this sense, the decision of CPU and motherboard are tied together and may influence each other.
  ...
  - Motherboards with the X chipset usually have better expandability as it's paired with Intel Core X-series HEDT CPUs, compared to those with the Z chipset paired with the regular Intel Core CPUs. They both support CPU overclocking. As an example:
    ...
    | ------------------------------------------ | ------------------------------------------------------------ | ---------------------------------------------------- |
    | Number of PCIe links and bandwidth of each | CPU: 2 PCIe 3.0 x16<br />Chipset: 1 PCIe 3.0 x16, 3 PCIe 3.0 x1 | CPU: 3 x PCIe 3.0 x16<br />Chipset: 1 x PCIe 3.0 x4  |
...
- Given the domain specialty, not many application can be accelerated by the GPU. In fact, most apps that are frequently associated with graphics-related aren't getting much of a performance boost by a dedicated GPU (at least for now - the companies are working towards the optimization). Also, programming on GPU follows a very different pattern from our regular devoping of programs running on CPU. (If you're curious, try writing a program in OpenCL that calculates matrix multiplication on GPU and uses all the GPU's cores.) There's a bit of a learning curve there, so we expect these apps' GPU optimizations will take some time. 
...
- Choose between Nvidia and AMD.
  ...
  - Benchmarks show many professional apps (Ex. Adobe Premiere Pro, Adobe Photoshop CC) have better performance on Nvidia GPUs than the same-level AMD GPUs due to better software optimization. Check out Puget Systems for more details. So unless there's any special requirement or compatibility issue, Nvidia would be the preferable choice.
- Go to a benchmark site to find performance ranking of all GPU on the market and find one that best suits your need.
  - [GPU Hierarchy by Tom's Hardware](https://www.tomshardware.com/reviews/gpu-hierarchy,4388.html)
  - An example of latest-generation Nvidia GPUs: Nvidia Titan RTX, Nvidia RTX 2080 Ti, Nividia RTX 2080 Super
- Pay attention to distinguish between a GPU and a Graphics Card. GPU is only one part - the most important part - of a graphics card (think of it as a "GPU chip" to better understand it.) Besides this chip, the Graphics Card also contains dedicated memory, a "motherboard" with data transmission links between the GPU and memory, etc. Nvidia and AMD are the only two major companies for GPU. They manufacture Graphics Cards themselves. but also authorize the use of these GPUs to a large number of other hardware companies. All these companies can then attach the other components and manufacturer the entire graphics card. Thus, we need to choose a particular model of the Graphics Card from a brand.
  - The three high-end motherboard brands all manufactuer premium-level Nvidia graphics cards, so we're sticking with the brand of our motherboard for best software compatibility. Some models to consider as an example:
...
- Decide the total RAM capacity. This can be based on your observation on how your current system runs, to see how much upgrade you need.
  ...
  - The size of RAM is the performance bottleneck of my current laptop - there're frequently more than 8 GB of swap used with the available 16 GB RAM. 
  - Many applications that we'll be using are very memory-intensive, including stiching panoramaic pictures in Adobe Photoshop from RAW-format images (which easily takes tens of Gigs of RAM) and running multiple virtual machines. So we decide to give it a jump to a total of 96 GB.
...
- Look at the hardware spec of a RAM stick and find the desired model number.
  ...
  - Below are the spec of some Corsair RAM modules that we've considered:
    ...
    Note that the RAM's data access latency can be best measured with \(\frac{\texttt{Frequency}}{\texttt{LAS Latency}}\) - that is, the number of data access per second. After doing the calculation, we see that this metric is actually very similar for all the modules above, though they appear to have very different frequencies. 
    
  - Thus, we use the following strategy to find the desired model.
  
    - Browsing available models with a relatively low LAS latency, such as 15 and 16. To achieve the same performance, a low latency is slightly better than a high frequency in my opinion, because overclocking to such a high frequency may result in a less reliable system. Among those modules, find one that supports relatively high maximum frequency, such as 3200 MHz.
...
- Should I use 1 larger SSD or 2 smaller SSDs?
  ...
  - Keep in mind of the caveats of this configuration: one more component simply means one more risk of failure. The data will be lost if either of the drive fails. Also further improvement on the bandwidth may not easily observable unless in very data-intensive applications, considering the high I/O performance of a single modern SSD.
  - A single SSD brings more flexibility for partitions. For example, creating two 750 GB partions and one 500 GB partition is easier on one 2 TB drive than two 1 TB drives. Of course we can create a logical volume across the two physical drives with the same result, but that's just some more hassle. 
...
- Should I use air cooling or liquid cooling?
  ...
  - Intel recommends a liquid cooling system for higher-end CPUs such as a Core i9-9900K. But the difference may not be that huge as we imagined: a high-end air cooling system can bring the CPU to a lower temperature than a mid-end liquid cooling system. The CPU is not running under full load most of the time anyways, so the cooling system usually doesn't get the chance to exert its full power.
  - Liquid cooling may have a slightly higher noise level.
  - A liquid cooling system "wraps" the CPU so its heat is self-contained inside the cooler and not dissipated into the case. So other air-cooled components (such as the GPU) are working in a cooler environment and this cooling system for CPU indirectly helps the performance of those components. This is an interesting observation that we haven't thought of in the first place.
- You can either purchase a complete liquid cooling system (closed-loop system) or build your own from scratch (open-loop system).
  - A visual example of two systems on the left and right, respectively:![diy-pc-liquid-cooling](/Users/max/Documents/CS-Projects/maxwyb.github.io/assets/diy-pc-liquid-cooling.png)
  - Installing the pump, connecting the tubes and filling water requires careful testing - there could be some risks of leaking water and damaging hardware due to inperfect installation. Commercial liquid cooling systems may provide warranty for all your hardware in case the water gets leaked. 
...
- Pay attention to the following parameters:
  - Type of Fan blade: Ex. Corsair manufacuters cooling system with 4+ types of blades, where ML is believed to be one of the most effective option (compared to LL, etc.)
...
- Pay attention to the following specs:
  ...
  - Number of 4-pin ATX power connectors: make sure the PSU has enough of those to plug into all available connectors on the motherboard for high power demand when running on full load or for overclocking. [PC Part Picker](https://pcpartpicker.com/list/) helps check this compatibility after you add all desired parts.
...
- Choose a size to fit your motherboard (using ATX as an example): Mid Tower VS. Full Tower
  - Some RAM modules with a "shell" (such as the Corsair Vengeance RGB Pro) are higher than barebone RAM sticks, and may touch with the two top case fans. The top case fans may be a very tough fit or have to be installed in alternative locations. Overall the hardware would fit very tightly in the case. Check out some YouTube videos on the size of the motherboard and the case to get a feel.
...
- First, design the airflow inside the case. Then decide if you need to buy additional fans besides those that come with the case, and how many if the answer is yes.
  - ![diy-pc-airflow-design](/Users/max/Documents/CS-Projects/maxwyb.github.io/assets/diy-pc-airflow-design.jpeg)
...
- Specs to consider when selecting a case fan
  - Size of the fan: larger fans require less RPM to move the same volume of air, so it's quieter than smaller fans. For example, either 3 120mm fans or 2 140mm fans can be installed on top of our case. We decide to go with the largest fan that can be installed on the case.
